...
function copy_files()
{
    ...
    cp ${PWD}/.vimplus/.vimrc.custom.plugins ~

    rm -rf ~/.vimrc.custom.config
    cp ${PWD}/.vimplus/.vimrc.custom.config ~

    rm -rf ~/.ycm_extra_conf.py
    ln -s ${PWD}/.vimplus/.ycm_extra_conf.py ~
...
function install_fonts_on_mac()
{
    ...
    cp ./.vimplus/fonts/Droid\ Sans\ Mono\ Nerd\ Font\ Complete.otf ~/Library/Fonts
}
...
function install_fonts_on_android()
{
    ...
    mkdir ~/.termux
    cp ./.vimplus/fonts/DejaVu.ttf ~/.termux/font.ttf
...
function main()
{
    ...
    echo "#############################################################################################"
    read -p "Ready to install vimplus, dou you want to install it? [Y/N] " ch
    if [[ $ch == "N" ]] || [[ $ch == "n" ]]; then
        echo "vimplus is installed failed!"
        return
    fi
    ...
    echo -e "\n"
    echo "#############################################################################################"
    echo "Vimplus (https://github.com/chxuan/vimplus) has been installed successfully, just enjoy it!"
    echo "#############################################################################################"
    echo -e "\n"
}
